...
## Troubleshooting
- If dataset parsing fails, check file encoding and format (JSON must be an array of objects).
- If signup/login fail, verify the database connection and required environment variables in backend/db.py and backend/security.py.
- If frontend cannot connect, confirm the backend is running and CORS origins include the frontend origin (backend/main.py allows localhost:5173).

## License & use
This repository is provided as a learning/demo project. Review source files in `backend/` and `frontend/` before deploying. No contribution or contribution workflow is described here.

---

If you want, I can:
- produce a ready-to-save README.md file with this content, or
- generate minimal run scripts (docker-compose / Makefile) to simplify running both services locally.
